function onCreate()
	for i = 0, getProperty('unspawnNotes.length')-1 do
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Glitch Note' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'funkmon/Static_Note_assets'); --Change texture
			setPropertyFromGroup('unspawnNotes', i, 'noteSplashTexture', 'funkmon/Glitch_Splash_assets');
...
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'Glitch Note' then
		makeAnimatedLuaSprite('image','funkmon/GlitchingEffect',0,0)addAnimationByPrefix('image','dance','Glitching',24,true)
